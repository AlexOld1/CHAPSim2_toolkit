...
quick_turb_stats.py: Single case post-processing script, currently outputs a figure for velocity/TKE/Temperature and a figure for Reynolds stresses. Ideal for use on Archer2 with only numpy, matplotlib and tqdm dependencies. Recommended to be run on a serial/ data analysis node as bandwidth is throttled on login nodes. Interactive input.
